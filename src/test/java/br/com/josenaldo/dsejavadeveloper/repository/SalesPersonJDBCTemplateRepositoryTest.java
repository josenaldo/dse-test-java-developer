...
@IntegrationTest
@Testcontainers
class SalesPersonJDBCTemplateRepositoryTest {

  @Autowired
  private SalesPersonJDBCTemplateRepository repository;
  ...
  void setUp() {
    ...
    assertThat(5).isEqualTo(repository.count());
  }
  ...
  void givenAValidSalary_whenFindBiggerSalaries_thenShouldReturnTwoSalesPerson(){
    ...
    // Act - When
    final var result = repository.findBiggerSalaries(expectedBaseSalary);
  ...
  void givenSalesPersonList_whenFindOlderThan60_thenShouldReturnOneSalesPerson(){
    ...
    // Act - When
    final var result = repository.findOlderThan60();
